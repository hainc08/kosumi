import type { Worker } from '@/types'

export const seedWorkers: Worker[] = [
  {
    id: 'w-1', code: 'CN001', fullName: 'Nguyễn Văn Hùng', gender: 'male',
    dateOfBirth: '1990-05-12', idNumber: '001090012345', phone: '0901234567',
    address: 'Đông Anh, Hà Nội', position: 'worker',
    specialty: 'Hàn kết cấu', status: 'working', notes: null,
    createdAt: '2026-01-12T00:00:00Z', updatedAt: '2026-01-12T00:00:00Z',
    initials: 'NH', avatarColor: '#1D4ED8',
    activeContract: {
      id: 'c-1', workerId: 'w-1', contractType: 'official', startDate: '2026-01-12',
      endDate: null, baseSalary: 12000000,
      allowanceResponsibility: 800000, allowanceAttendance: 500000,
      ratePerUnit: null, unitName: null, isActive: true,
      createdAt: '2026-01-12T00:00:00Z', updatedAt: '2026-01-12T00:00:00Z',
    },
  },
  {
    id: 'w-2', code: 'CN002', fullName: 'Trần Thị Mai', gender: 'female',
    dateOfBirth: '1995-09-03', idNumber: '001195067890', phone: '0912345678',
    address: 'Long Biên, Hà Nội', position: 'worker',
    specialty: 'Lắp dựng thép', status: 'working', notes: null,
    createdAt: '2026-02-02T00:00:00Z', updatedAt: '2026-02-02T00:00:00Z',
    initials: 'TM', avatarColor: '#16A34A',
    activeContract: {
      id: 'c-2', workerId: 'w-2', contractType: 'official', startDate: '2026-02-02',
      endDate: null, baseSalary: 9000000,
      allowanceResponsibility: null, allowanceAttendance: 300000,
      ratePerUnit: null, unitName: null, isActive: true,
      createdAt: '2026-02-02T00:00:00Z', updatedAt: '2026-02-02T00:00:00Z',
    },
  },
  {
    id: 'w-3', code: 'CN003', fullName: 'Lê Văn Tâm', gender: 'male',
    dateOfBirth: '1988-12-20', idNumber: '001088054321', phone: '0987654321',
    address: 'Gia Lâm, Hà Nội', position: 'team_leader',
    specialty: 'Quản lý tổ thép', status: 'on_leave', notes: 'Nghỉ phép năm',
    createdAt: '2026-01-15T00:00:00Z', updatedAt: '2026-05-01T00:00:00Z',
    initials: 'LT', avatarColor: '#D97706',
    activeContract: {
      id: 'c-3', workerId: 'w-3', contractType: 'official', startDate: '2026-01-15',
      endDate: null, baseSalary: 15000000,
      allowanceResponsibility: 2000000, allowanceAttendance: 500000,
      ratePerUnit: null, unitName: null, isActive: true,
      createdAt: '2026-01-15T00:00:00Z', updatedAt: '2026-01-15T00:00:00Z',
    },
  },
  {
    id: 'w-4', code: 'CN004', fullName: 'Phạm Đình Quân', gender: 'male',
    dateOfBirth: '1992-03-08', idNumber: '001092011223', phone: '0934567890',
    address: 'Sóc Sơn, Hà Nội', position: 'worker',
    specialty: 'Gia công cơ khí', status: 'working', notes: null,
    createdAt: '2026-01-20T00:00:00Z', updatedAt: '2026-01-20T00:00:00Z',
    initials: 'PQ', avatarColor: '#7C3AED',
    activeContract: {
      id: 'c-4', workerId: 'w-4', contractType: 'probation', startDate: '2026-04-01',
      endDate: '2026-07-01', baseSalary: 7000000,
      allowanceResponsibility: null, allowanceAttendance: 300000,
      ratePerUnit: null, unitName: null, isActive: true,
      createdAt: '2026-04-01T00:00:00Z', updatedAt: '2026-04-01T00:00:00Z',
    },
  },
  {
    id: 'w-5', code: 'QL001', fullName: 'Hoàng Minh Khải', gender: 'male',
    dateOfBirth: '1975-07-25', idNumber: '001175078899', phone: '0945678901',
    address: 'Hoàng Mai, Hà Nội', position: 'director',
    specialty: 'Quản trị doanh nghiệp', status: 'working', notes: null,
    createdAt: '2026-02-10T00:00:00Z', updatedAt: '2026-02-10T00:00:00Z',
    initials: 'HK', avatarColor: '#DB2777',
    activeContract: {
      id: 'c-5', workerId: 'w-5', contractType: 'official', startDate: '2026-01-01',
      endDate: null, baseSalary: 35000000,
      allowanceResponsibility: 5000000, allowanceAttendance: null,
      ratePerUnit: null, unitName: null, isActive: true,
      createdAt: '2026-01-01T00:00:00Z', updatedAt: '2026-01-01T00:00:00Z',
    },
  },
  {
    id: 'w-6', code: 'CN006', fullName: 'Đỗ Minh Khang', gender: 'male',
    dateOfBirth: '1985-11-02', idNumber: '001085033445', phone: '0956789012',
    address: 'Bình Tân, TP.HCM', position: 'foreman',
    specialty: 'Giám sát thi công', status: 'working', notes: null,
    createdAt: '2026-01-18T00:00:00Z', updatedAt: '2026-01-18T00:00:00Z',
    initials: 'ĐK', avatarColor: '#0891B2',
    activeContract: {
      id: 'c-6', workerId: 'w-6', contractType: 'official', startDate: '2026-01-18',
      endDate: null, baseSalary: 18000000,
      allowanceResponsibility: 3000000, allowanceAttendance: 500000,
      ratePerUnit: null, unitName: null, isActive: true,
      createdAt: '2026-01-18T00:00:00Z', updatedAt: '2026-01-18T00:00:00Z',
    },
  },
  {
    id: 'w-7', code: 'QL002', fullName: 'Vũ Thị Hồng', gender: 'female',
    dateOfBirth: '1983-04-17', idNumber: '001183044556', phone: '0967890123',
    address: 'Bình Tân, TP.HCM', position: 'chief_accountant',
    specialty: 'Kế toán tài chính', status: 'working', notes: null,
    createdAt: '2026-02-15T00:00:00Z', updatedAt: '2026-02-15T00:00:00Z',
    initials: 'VH', avatarColor: '#CA8A04',
    activeContract: {
      id: 'c-7', workerId: 'w-7', contractType: 'official', startDate: '2026-02-15',
      endDate: null, baseSalary: 20000000,
      allowanceResponsibility: 2000000, allowanceAttendance: 500000,
      ratePerUnit: null, unitName: null, isActive: true,
      createdAt: '2026-02-15T00:00:00Z', updatedAt: '2026-02-15T00:00:00Z',
    },
  },
  {
    id: 'w-8', code: 'CN008', fullName: 'Bùi Văn Sơn', gender: 'male',
    dateOfBirth: '1991-08-30', idNumber: '001091055667', phone: '0978901234',
    address: 'Long Biên, Hà Nội', position: 'worker',
    specialty: 'Sơn tĩnh điện', status: 'absent', notes: 'Nghỉ không phép 1 ngày',
    createdAt: '2026-02-20T00:00:00Z', updatedAt: '2026-06-14T00:00:00Z',
    initials: 'BS', avatarColor: '#DC2626',
    activeContract: {
      id: 'c-8', workerId: 'w-8', contractType: 'piece_rate', startDate: '2026-02-20',
      endDate: null, baseSalary: null,
      allowanceResponsibility: null, allowanceAttendance: null,
      ratePerUnit: 150000, unitName: 'm²', isActive: true,
      createdAt: '2026-02-20T00:00:00Z', updatedAt: '2026-02-20T00:00:00Z',
    },
  },
]

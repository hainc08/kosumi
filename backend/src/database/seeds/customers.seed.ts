...
export async function seedCustomers(ds: DataSource): Promise<void> {
  ...
  // Ported from frontend/src/mocks/seed/customers.ts
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const saved = await repo.save(repo.create({ ...customer, code: makeCode('KH', i + 1) }))
